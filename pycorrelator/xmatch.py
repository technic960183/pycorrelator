...
def xmatch(catalog1: pd.DataFrame, catalog2: pd.DataFrame, tolerance, retain_index=False, inplace=False, verbose=True):
    """
    Purpose: This function performs a cross-match between two catalogs.
    Parameters:
        - catalog1 (DataFrame): The first catalog.
        - catalog2 (DataFrame): The second catalog.
    ...
    catalog1 = Catalog(catalog1).get_dataframe()
    catalog2 = Catalog(catalog2).get_dataframe()
    ...
    cg1.distribute(catalog1)
    cg2.distribute(catalog2)
    ...
    return XMatchResult(catalog1, catalog2, tolerance, merged_dict)
